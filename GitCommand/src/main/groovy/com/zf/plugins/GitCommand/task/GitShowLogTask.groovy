...
import org.gradle.api.tasks.TaskAction
 ...
 class GitShowLogTask extends DefaultTask {
    ...
    @TaskAction
    void action() {
        String cmd = "git log --date=format:\"%m-%d %H%M:%S\" --pretty=format:\"%h - %an, %ad : %s\" -5"
     ...
     static class CreationAction implements Action<GitShowLogTask> {
        ...
        @Override
        void execute(GitShowLogTask task) {
            configWorkDir(task)
        }

        def  configWorkDir(GitShowLogTask task) {
